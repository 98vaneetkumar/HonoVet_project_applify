...
function resetPassword(email, userType, env) {
  ...
  if (!newPassword) {
    ...
    $("#newPasswordErrDiv")
      .show()
      .html(
        "New Password should not allows blank spaces. Please enter a valid password"
      )
      .css("color", "red");
    return false;
    ...
    $("#newPasswordErrDiv")
      .show()
      .html("New Password should be between 8 to 16 characters")
      .css("color", "red");
    return false;
  } else if (!confirmPassword) {
    $("#newPasswordErrDiv").html("");
    $("#confirmPasswordErrDiv")
      .show()
    ...
    $("#confirmPasswordErrDiv")
      .show()
      .html(
        "Confirm Password should be a minimum of 8 characters and a maximum of 16 characters"
      )
      .css("color", "red");
    return false;
    ...
    //Add this code to run the reset password
    var baseUrlAdmin = "http://192.168.1.13:3002/";
    var url = baseUrl + "";
    if (userType == "admin") {
      url = baseUrlAdmin + "profile/reset-password";
      // url = "http://localhost:3002" + '/api/v1/admin/admins/resetNewPassword';
    }
    if (userType == "user") {
      url = baseUrl + "profile/resetEmailPassword";
    }
    var data = {
      email: email,
    ...
    $.ajax({
      ...
      error: function (XMLHttpRequest, textStatus, errorThrown) {
        ...
        if (
          ...
          $("#confirmPasswordErrDiv")
            .html(XMLHttpRequest.responseJSON.message)
            .css("color", "red");
        } else {
          $("#confirmPasswordErrDiv")
      ...
      success: function (data) {
        if (userType == "admin") {
          location.href = baseUrlAdmin + "profile/password-success";
        } else {
