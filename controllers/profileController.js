...
module.exports = {
  addUser: async (payloadData, path) => {
    ...
    let checkProfile = await Service.profileServices.getProfile(
      condition,
    ...
    if (addProfile) {
      let path = "/profile/generatePassword?email=" + payload.email + "&token=";
      var variableDetails = {
      ...
      await NotificationManager.sendMail(
        "PROFILE_HONOR",
        payload.email,
        variableDetails
      );
  ...
  createProfile: async (payloadData) => {
    const schema = Joi.object().keys({
      ...
      serviceDisabled: Joi.number().valid(0, 1).optional(),
      myPersonalities: Joi.string().optional().allow(""),
      ...
      servicehonorsAwards: Joi.string().optional(),
      vegetranServiceDisabled: Joi.number().valid(0, 1).optional(),
    ...
    if (_.has(payload, "Pronouns") && payload.Pronouns != "")
      objToSave.Pronouns = payload.Pronouns;
    if (_.has(payload, "Bio") && payload.Bio != "") objToSave.Bio = payload.Bio;
    if (_.has(payload, "phoneNumber") && payload.phoneNumber != "")
      objToSave.phoneNumber = payload.phoneNumber;
    if (_.has(payload, "dateOfBirth") && payload.dateOfBirth != "")
      objToSave.dateOfBirth = payload.dateOfBirth;
    if (_.has(payload, "Gender") && payload.Gender != "")
      objToSave.Gender = payload.Gender;
    if (_.has(payload, "HonorsAwards") && payload.HonorsAwards != "")
      objToSave.HonorsAwards = payload.HonorsAwards;
    if (_.has(payload, "serviceDisabled") && payload.serviceDisabled != "")
      objToSave.serviceDisabled = payload.serviceDisabled;
    if (_.has(payload, "myPersonalities") && payload.myPersonalities != "")
      objToSave.myPersonalities = payload.myPersonalities;
    if (_.has(payload, "currentLocation") && payload.currentLocation != "")
      objToSave.currentLocation = payload.currentLocation;
    if (_.has(payload, "location") && payload.location != "")
      objToSave.location = payload.location;
    if (_.has(payload, "zipCode") && payload.zipCode != "")
      objToSave.zipCode = payload.zipCode;
    if (_.has(payload, "document") && payload.document != "")
      objToSave.document = payload.document;
    if (
      _.has(payload, "marriageCertificate") &&
      payload.marriageCertificate != ""
    )
      objToSave.marriageCertificate = payload.marriageCertificate;
    if (_.has(payload, "spouseName") && payload.spouseName != "")
      objToSave.spouseName = payload.spouseName;
    if (_.has(payload, "service") && payload.service != "")
      objToSave.service = payload.service;
    if (_.has(payload, "jobTitle") && payload.jobTitle != "")
      objToSave.jobTitle = payload.jobTitle;
    if (
      _.has(payload, "servicehonorsAwards") &&
      payload.servicehonorsAwards != ""
    )
      objToSave.servicehonorsAwards = payload.servicehonorsAwards;
    if (
      _.has(payload, "vegetranServiceDisabled") &&
      payload.vegetranServiceDisabled != ""
    )
      objToSave.vegetranServiceDisabled = payload.vegetranServiceDisabled;
    if (_.has(payload, "termCondition") && payload.termCondition != "")
      objToSave.termCondition = payload.termCondition;
    if (_.has(payload, "isDeleted") && payload.isDeleted != "")
      objToSave.isDeleted = payload.isDeleted;

    let addProfile = await Service.profileServices.updateData(
      condition,
      objToSave
    );
    if (addProfile) {
  ...
  getProfileById: async (paramData) => {
    ...
    let profile = Service.profileServices.getProfile(
      criteria,
      profileProjection,
      true
    );
  ...
  validateToken: async (payloadData) => {
    ...
    if (!payload || !payload.token) {
      ...
      let data = await Service.profileServices.getProfile(
        criteria,
        ["id", "email", "Name"],
        false
      );
  ...
  resetNewPassword: async (payloadData) => {
    ...
    let admin = await Service.profileServices.getProfile(
      criteria,
    ...
    if (admin) {
      ...
      if (adminObj && adminObj.id) {
        ...
        let update = await Service.profileServices.updateData(
          criteria,
          objToSave
        );
        if (update) {
  ...
  login: async (payloadData) => {
    ...
    let checkEmailExist = await Service.profileServices.getProfile(
      emailCriteria,
      projection,
      true
    );
  ...
  forgotPassword: async (payloadData) => {
    ...
    let adminData = await Service.profileServices.getProfile(
      criteria,
      ["email", "Name", "id", "forgotPasswordGeneratedAt", "isBlocked"],
      false
    );
    adminObj = adminData;
    if (!adminObj) {
      ...
      adminObj &&
      adminObj.forgotPasswordGeneratedAt &&
    ...
    let objToSave = {
      forgotPasswordGeneratedAt: moment(new Date(Date.now())).format(
        "YYYY-MM-DD HH:mm:ss"
      ),
    ...
    let path =
      "/veteran/v1/honorvet/generatePassword?email" + payload.email + "&token=";
    var variableDetails = {
  ...
  validateToken: async (payloadData) => {
    ...
    if (!payload || !payload.token) {
      ...
      criteria.passwordResetToken = payload.token;
      let data = await Service.profileServices.getAdmin(
        criteria,
        ["id", "email", "Name"],
        false
      );
